...
void kissat_release(kissat *solver)
{
  ...
  if (solver->ga != nullptr)
  {
    solver->ga->~GeneticAlgorithm();
    ::operator delete(solver->ga);
    solver->ga = nullptr; // Optional: Set the pointer to nullptr for safety.
  }
    ...
    else
    {
      ...
      if (!isize)
      {
        ...
        if (!solver->inconsistent)
        {
          ...
          // std::cout << "inconsistent dans internal.cc::kissat_add" << std::endl;
          CHECK_AND_ADD_EMPTY();
          ADD_EMPTY_TO_PROOF();
        }
      }
      else if (isize == 1)
      {
        ...
        if (!solver->level)
        {
          ...
          if (conflict)
          {
            ...
            solver->inconsistent = true;
            // std::cout << "inconsistent dans internal.cc::kissat_add 412" << std::endl;
            CHECK_AND_ADD_EMPTY();
            ADD_EMPTY_TO_PROOF();
          }
        }
