...
parse_options(application *application, int argc, char **argv)
{
  ...
  for (int i = 1; i < argc; i++)
  {
    ...
    else if ((valstr = kissat_parse_option_name(arg, "population")))
    {
      int val;
      if (kissat_parse_option_value(valstr, &val) && val >= 0)
        solver->population_size = val;
    }
    else if ((valstr = kissat_parse_option_name(arg, "generations")))
    {
      int val;
      if (kissat_parse_option_value(valstr, &val) && val >= 0)
        solver->max_generations = val;
    }
    else if ((valstr = kissat_parse_option_name(arg, "mutation")))
    {
      int val;
      if (kissat_parse_option_value(valstr, &val) && val >= 0)
        solver->mutation_rate = val / 100.0;
    }
    else if ((valstr = kissat_parse_option_name(arg, "crossover")))
    {
      int val;
      if (kissat_parse_option_value(valstr, &val) && val >= 0)
        solver->crossover_rate = val / 100.0;
    }
